...
look_from_initial = (13.0, 2.0, 3.0)
...
# --- Helper functions ---
#  param pos - takes an array [x,y,z] for the position the sphere will be placed
# param radius - float for the size of the sphere
# param material - material the sphere will be created with, 
...
#material type physicallyBased
# param ior (index of refraction) - float 
...
# params rgb - floats, color picker
# param ior (index of refraction) - float
# param transmission_amount - float bounded 0-1,  0 = solid ,1 = clear
...
# param albedo - float of array 3, base color, first 2 params bounded [-1,1], third bounded (0,1)
# param fuzz - float, default 1
...
def create_spheres():
    ...
    add_sphere((0.,-100.,0.),10.,make_lambertian(.5,.5,.5)) # down sphere
    add_sphere((0.,100.,0.),10.,make_lambertian(.5,.5,.5)) # up sphere
    add_sphere((100., 1., 0.), 10., make_metal((.7,.7,.5), 0.)) # east sphere
    add_sphere((-100., 1., 0.), 10., make_colored_glass(1.0,.95,.0,1.1,.9)) #west sphere
    add_sphere((0., 1., 100.), 10., make_dielectric(.5)) #north sphere
    add_sphere((0.,-1.,-100.),10.,make_lambertian(.2,.2,.2)) # south sphere
    ...
    #picks random locations and materials to be placed in the world, always of height 0.2
...
# --- ONE-TIME SETUP ---
device = anari.newDevice('default') #used to combine everything and share values
fig, ax = plt.subplots()
spheres, movable_geoms, sphere_anim_params = [], [], [] 

create_spheres() #populating spheres, movable_geoms, and sphere_anim_params

world = device.newWorld()
world.setParameterArray1D('surface', anari.SURFACE, spheres )
world.commitParameters() #putting sphere arrays into the world

bg_values = np.array(((.9,.9,.9,1.),(.15,.25,.8,1.)), dtype=np.float32).reshape((2,1,4)) # tuple size 2 (color 1 (rgba), color 2(rgba)) reshape(row, col, dimensions)
bg_gradient = device.newArray2D(anari.float4, bg_values) 
renderer = device.newRenderer('default')
renderer.setParameter('pixelSamples', anari.INT32, num_paths_per_pixel) # image fidelity, higher paths more smooth
renderer.setParameter('background', anari.ARRAY2D, bg_gradient)
renderer.commitParameters() #creating baground

camera = device.newCamera('perspective')
 #for perspective camera
camera.setParameter('aspect', anari.FLOAT32, fb_size[0]/fb_size[1]) #aspect ratio
camera.setParameter('fovy', anari.FLOAT32, fovy * np.pi / 180) #fov

# for Omnidirectional - but is currently not working
# camera.setParameter('layout', anari.STRING, 'equirectangular')  # default parameter

frame = device.newFrame() # puts all the components together onto one frame, top level container
...
class CameraState:
    def __init__(self, look_from, look_at):
        self.look_at = look_at # coordinates to look 
        vec = np.array(look_from) - self.look_at #position to those coordinates from current position
        self.radius = np.linalg.norm(vec) #distance
        self.azimuth = np.arctan2(vec[2], vec[0]) # x-axis rotation
        self.elevation = np.arcsin(vec[1] / self.radius) # y-axis rotation
        self.is_dragging = False #mouse currently dragging
...
def on_scroll(event):
    ...
    cam_state.radius = max(2.0, cam_state.radius) # type: ignore
...
def update(frame_num):
    ...
    for i, geom in enumerate(movable_geoms):
        ...
        new_pos_array = device.newArray1D(anari.FLOAT32_VEC3, np.array([new_pos], dtype=np.float32))
        geom.setParameter('vertex.position', anari.ARRAY1D, new_pos_array)
        geom.commitParameters()
